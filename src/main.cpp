...
uint256 hashGenesisBlock("0x727c0d2dc832a7eea60f6bbb858466235bbab710a8493d3e078e6f69c34a5870");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // UMCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "UMCoin Signed Message:\n";
...
{
    // UMCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // UMCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // UMCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // UMCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x727c0d2dc832a7eea60f6bbb858466235bbab710a8493d3e078e6f69c34a5870");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1412181260;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084736964;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UMCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UMCoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running UMCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
